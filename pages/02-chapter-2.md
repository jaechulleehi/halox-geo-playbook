...
## 1장 산출물이 2장 측정표로 바뀌는 방식

2장은 1장에서 만든 SEO 산출물을 그대로 이어받습니다. 키워드 리서치에서 나온 query는 AI 질문셋의 후보가 되고, SERP 분석에서 확인한 검색 의도는 질문군 분류의 기준이 됩니다. 온페이지 SEO와 내부 링크 점검 결과는 citation 후보 URL을 고르는 기준이 되고, 권위/엔티티 점검 결과는 source 후보를 해석하는 기준이 됩니다.

| 1장 산출물 | 2장에서 바뀌는 측정 항목 | 확인 질문 |
|---|---|---|
| 핵심 query 목록 | 브랜드/비브랜드 질문셋 | 실제 사용자가 AI에게 물을 문장으로 바뀌었는가? |
| SERP 분석 노트 | 플랫폼별 답변 유형 | 검색결과와 AI 답변이 같은 의도를 다루는가? |
| 검색 의도 분류 | 정의형/비교형/추천형/검증형/실행형 질문군 | 구매 여정별로 질문이 고르게 들어갔는가? |
| 콘텐츠 브리프 | citation 후보 URL | 답변에 인용되었을 때 바로 쓸 수 있는 구조인가? |
| 내부 링크 맵 | source/citation 안정성 | 핵심 URL이 사이트 안에서 고립되어 있지 않은가? |
| 권위/엔티티 점검 | 외부 source 후보 | 외부에서 브랜드가 같은 카테고리로 설명되는가? |
| GSC/GA4 데이터 | 검색성과와 AI 노출의 연결 | AI 답변 변화가 검색 유입/전환 변화와 함께 보이는가? |

이 연결이 있어야 2장이 단순 지표 설명에서 끝나지 않습니다. 측정표는 `AI가 우리를 알고 있나?`를 묻는 표가 아니라, `어떤 SEO/GEO 자산을 고치면 다음 측정에서 달라질까?`를 찾는 운영 도구가 됩니다.
...
이 표를 기준으로 보면 2장의 측정은 1장의 SEO 기본기와 다시 연결됩니다. 측정에서 발견한 문제는 콘텐츠, 출처, 기술, 메시지 중 어디를 고칠지로 번역되어야 합니다.
