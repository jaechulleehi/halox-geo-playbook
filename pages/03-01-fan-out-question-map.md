...
| 경쟁 도구와 어디가 달라지는가 | 비교표/사례 | 비교 문서 작성 |
...
Fan-out 질문맵은 한 번에 많이 뽑는 것이 목적이 아닙니다. 먼저 검색 키워드 5~10개를 고르고, 각 키워드가 정의/비교/검증/구매 전 질문으로 어떻게 갈라지는지 봅니다. 이후 HaloX의 `프롬프트 분석`이나 유사한 모니터링 화면에서 실제 답변 문맥을 확인합니다.

| 단계 | 남길 것 |
|---|---|
| Seed 키워드 | 시장에서 이미 쓰는 검색어 |
| 확장 질문 | AI에게 실제로 던질 자연어 질문 |
| 질문 유형 | 정의/비교/검증/구매 전/문제 해결 |
| 현재 답변 | 브랜드 mention, 경쟁 브랜드, 반복 source |
| 다음 액션 | 새 글, 기존 글 수정, source 보강, 기술 점검 |

이 표가 있으면 콘텐츠팀은 무엇을 써야 하는지 알고, 리포트 담당자는 다음 측정 조건을 고정할 수 있습니다.
...
이전 흐름은 [03. Query Fan-out: AI가 내부에서 확장하는 질문 패턴](https://wikidocs.net/346343)에서 확인할 수 있습니다. 다음은 [03-02. GEO 질문셋과 AI 내부 질문 패턴 분리](https://wikidocs.net/346345)에서 측정 질문셋과 fan-out 패턴을 나눠 설계합니다.
