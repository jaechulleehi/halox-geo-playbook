...
여기서 `URL 확인 노드`를 봐야 합니다. 검색형 AI나 브라우징형 답변은 때로 공식 사이트, 제품 페이지, 고객사 사례 URL, 외부 기사 같은 웹 자산을 확인하면서 답변을 구성합니다. 따라서 GEO 콘텐츠는 질문에 답하는 문장뿐 아니라, AI가 확인할 수 있는 공식 URL과 일관된 설명을 갖춰야 합니다.

![AI가 질문을 쪼개는 방식](../assets/images/body-figures/halox-geo-03-01-fan-out-question-map-codex-only.png)

## fan-out 맵을 그리는 실무 순서

fan-out 맵은 머릿속으로 그리면 흐려집니다. 실무에서는 하나의 seed question을 정하고, 답변 구조와 citation/source 단서를 기록한 뒤, 각 하위 판단을 자산 점검으로 연결해야 합니다.

1. 2장 기준선에서 약한 seed question을 고릅니다.
2. 같은 질문을 같은 조건으로 3회 이상 측정합니다.
3. 답변을 정의/비교/추천/검증/source/URL/실행/리스크 노드로 나눕니다.
4. 각 노드에서 등장한 기준, 경쟁사, 출처, 누락 표현을 적습니다.
5. 우리 사이트의 대응 URL, 외부 source, schema/canonical 상태를 붙입니다.
6. 노드별로 콘텐츠 갭, source 갭, 기술 갭, 메시지 갭을 표시합니다.
7. 영향도와 실행 난이도를 기준으로 다음 30일 과제를 고릅니다.
8. 수정 후 같은 seed question으로 재측정합니다.

| fan-out 노드 | 관찰할 단서 | 확인할 자산 | 다음 액션 |
|---|---|---|---|
| 정의 | 어떤 개념 설명으로 시작하는가 | glossary, 첫 문단, FAQ | 용어/카테고리 설명 정리 |
| 비교 | 어떤 기준으로 후보를 나누는가 | 비교표, 대안 페이지 | 기준표와 제외 기준 보강 |
| 추천 | 누구에게 맞는다고 말하는가 | use case, 고객 사례 | 대상 고객/도입 조건 명확화 |
| 검증 | 믿을 근거를 무엇으로 삼는가 | 뉴스룸, 리뷰, 외부 source | 신뢰 근거와 제3자 출처 확보 |
| citation | 어떤 URL이 보이는가 | title, canonical, internal link | citation 후보 URL 안정화 |
| 실행 | 어떤 순서를 제안하는가 | 체크리스트, 리포트 템플릿 | 30일 실행안 추가 |
