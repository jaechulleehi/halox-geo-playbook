...
| fan-out 노드의 source 갭 | 본문보다 근거 자산이 부족함 | 05장 답변 근거/엔티티 과제로 넘김 |
...
| 항목 | 리라이트 전 | 리라이트 후 |
|---|---|---|
| 첫 문단 | AI 검색 시대에는 GEO를 봐야 합니다. 콘텐츠 전략이 바뀌고 있습니다. | GEO 리라이트는 기존 글을 AI 답변에 쓰일 수 있는 정보 단위로 바꾸는 작업입니다. 첫 답변, 비교 기준, FAQ, source, schema 후보를 정리하고 같은 질문으로 재측정합니다. |
...
리라이트 후에도 다음 문제가 남으면 05장 출처/엔티티 작업으로 넘깁니다. 페이지 안의 구조만 고쳐서는 AI가 신뢰할 만한 근거망이 만들어지지 않는 경우입니다.

| 남은 문제 | 05장에서 볼 항목 | 다음 액션 |
|---|---|---|
| 답은 좋아졌지만 화면 인용이 약함 | source/citation 차이 | 답변 근거 후보 URL과 화면 인용 URL을 분리해 기록 |
| 브랜드가 다른 카테고리로 설명됨 | 엔티티 합의 신호 | 공식 한 줄 정의와 외부 프로필 문장 정렬 |
...
이전: [04-02. FAQ/표/schema는 언제 쓰는가](https://wikidocs.net/346348) / 다음: [05. 답변 근거, 화면 인용, 엔티티 전략](https://wikidocs.net/346333)
