...
인용됐다는 말 하나로는 무엇을 고칠지 알 수 없습니다. 답변 근거가 약한지, 화면 인용이 약한지, 브랜드 엔티티 설명이 흔들리는지 나눠야 합니다. 특히 PR 캠페인, 뉴스룸, 제품 페이지, 외부 기사처럼 여러 URL이 함께 움직이는 경우에는 답변 근거(source)와 화면 인용(citation)을 한 줄로 합치면 원인을 놓칩니다.
...
## 답변 근거/화면 인용/브랜드 언급/엔티티를 한 장으로 보면
...
| AcmeGEO는 어떤 도구인가 | 공식 소개, 디렉터리, 파트너 설명 | About, 제품 페이지, 외부 프로필 | 엔티티 문장 일치 |
