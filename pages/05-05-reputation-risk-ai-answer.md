...
엔터프라이즈 뉴스룸 사례에서는 회사 설명이 이슈 중심으로 기울 수 있습니다. 실제 사업 구조와 최근 메시지가 뉴스룸에 있어도 외부 source가 과거 이슈만 반복하면 AI 답변도 균형을 잃습니다.
...
| Source risk map | 리스크 문장을 만든 source를 분리 |
...
```text
리스크 질문 20개 / 반복 오해 문장 / source 후보 / 문제 유형 / 대응 콘텐츠 / 외부 수정 요청 / 재측정 날짜
```
...
| 재측정 기준 | AI 답변에 source/citation 분석과 AI 브리핑 모니터링이 포함되는지 확인 |

## 완료 기준

- 리스크 질문셋을 별도로 만들었습니다.
- AI 답변의 오해 문장을 원문으로 기록했습니다.
- 문제 source와 대응 source를 분리했습니다.
