...
| 층 | 핵심 신호 | 확인할 질문 | 산출물 |
|---|---|---|---|
| 발견 | sitemap, 내부 링크, href, llms.txt | AI/검색 시스템이 핵심 URL을 찾을 수 있는가 | 핵심 URL 목록과 발견 경로 |
| 접근 | robots.txt, robots meta, 응답 코드, 서버 안정성 | 크롤러가 막히지 않고 200/301로 접근하는가 | 차단/오류/리다이렉트 표 |
| 해석 | title, meta description, H1/H2, 본문 첫 문단, schema | 이 페이지가 어떤 질문의 답인지 분명한가 | 페이지 의미 일치표 |
| 재측정 | Search Console, Rich Results Test, HaloX 질문셋 | 수정 후 source/citation이 개선되는가 | 수정 전후 리포트 |

이렇게 나누면 `GEO가 약하다`는 말을 개발팀에 그대로 넘기지 않아도 됩니다. URL별로 “발견 문제인지, 접근 문제인지, 해석 문제인지, 측정 문제인지”를 분리할 수 있습니다.
...
| schema | FAQPage, Article, Product, Organization, Person/ProfilePage 정보가 구조화되는가 | 의미 해석 | 개발/콘텐츠 |
| 메타 정보 | title/meta description/canonical/robots meta가 본문과 일치하는가 | 대표 URL/주제 판단 | SEO/개발/콘텐츠 |
...
테크니컬 GEO 점검은 사이트 전체를 막연히 훑는 것보다 핵심 URL 10개를 먼저 고르는 편이 좋습니다. 2장 리포트에서 citation 후보가 되어야 하는 페이지, 4장에서 리라이트한 페이지, 5장에서 source로 정한 페이지를 우선 봅니다.

| URL 유형 | 먼저 볼 항목 | 실패 신호 | 다음 조치 |
|---|---|---|---|
| 제품/서비스 페이지 | title, canonical, schema, 내부 링크 | 설명은 좋은데 citation으로 안 잡힘 | Article/Product/Organization 신호 점검 |
| 블로그/가이드 | 초기 HTML, H2, 표, FAQ | 본문이 JS 뒤에만 있음 | 핵심 문단 SSR/정적화 |
| 뉴스룸/팩트시트 | 200 상태, 날짜, 내부 링크 | 오래된 보도자료만 source로 반복 | 최신 팩트시트 연결 |
| 리포트/자료 페이지 | sitemap, canonical, 파일 설명 | PDF만 있고 HTML 설명이 없음 | 요약 HTML과 다운로드 설명 추가 |
| 글로벌 페이지 | hreflang, locale, canonical | 한국어 URL이 영어 답변 source로 나옴 | 언어별 대표 URL 정렬 |

## 기본 점검표: 10분 안에 먼저 볼 항목

기술 점검을 처음 시작할 때는 복잡한 진단보다 아래 항목을 먼저 봅니다. 이 표는 전체 기술 감사 전에 핵심 URL이 발견/접근/해석 가능한지 빠르게 가르는 1차 필터입니다.
...
| 주요 대상 | 색인, 순위, 클릭 가능성 | 답변 근거성, 화면 인용 안정성, 엔티티 이해 |
...
| GEO 영향 | AI 접근/인덱싱/답변 근거(source), 화면 인용(citation), 엔티티 |
| 수정 담당 | 개발/마케팅/콘텐츠/PR |
...
겹치는 부분이 많습니다. 다만 GEO에서는 검색 순위뿐 아니라 AI 답변의 답변 근거(source)/화면 인용(citation), 엔티티 이해, answer quality까지 연결해 봅니다.
