...
## llms.txt와 사이트 이전을 함께 보는 이유

llms.txt는 AI가 읽으면 좋은 대표 문서와 정책을 안내하는 보조 신호입니다. 하지만 사이트 이전이나 URL 변경이 정리되지 않은 상태에서 llms.txt만 추가하면 효과가 제한됩니다. AI가 참고해야 할 핵심 URL, canonical, sitemap, 내부 링크가 함께 맞아야 합니다.

| 점검 항목 | 확인 질문 | 권장 처리 |
|---|---|---|
| 핵심 문서 목록 | AI가 먼저 읽을 가이드/정책/용어집이 정리되어 있는가 | llms.txt에 대표 문서만 선별 |
| URL 이전 | 예전 URL이 새 URL로 연결되는가 | 301 redirect와 canonical 정리 |
| sitemap 반영 | 새 URL이 sitemap에 들어갔는가 | sitemap 재생성/등록 |
| 내부 링크 | 본문 링크가 새 URL을 가리키는가 | 오래된 내부 링크 교체 |
| 메시지 일관성 | llms.txt 문서와 실제 본문이 같은 설명을 하는가 | 대표 문서 본문 업데이트 |

llms.txt는 마법 파일이 아니라, 이미 정리된 정보 구조를 AI에게 안내하는 목차에 가깝습니다.
