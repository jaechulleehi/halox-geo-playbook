...
## robots.txt와 sitemap은 실제로 어떻게 생겼나

robots.txt는 크롤러 트래픽을 관리하는 파일입니다. sitemap은 Google이 사이트를 더 똑똑하게 크롤링할 수 있도록 URL 정보를 제공하는 파일입니다. 둘 다 “노출 보장 장치”가 아니라 발견과 접근을 돕는 기본 신호입니다.
...
GEO에서 중요한 것은 `Allow`가 많다는 사실이 아닙니다. AI 답변에 근거로 쓰이길 원하는 글로서리, 블로그, 뉴스룸, 제품 문서가 실제로 sitemap과 내부 링크에서 발견 가능한지입니다.
...
Google은 링크를 통해 새 페이지를 발견하고 관련성을 판단한다고 설명합니다. GEO에서도 내부 링크는 단순 UX가 아니라 AI가 개념/제품/사례의 관계를 따라가는 길입니다.

| 링크 상태 | 좋은 예 | 위험한 예 |
|---|---|---|
| 실제 href | `<a href="/ko/blog/schema-markup-practical">스키마 마크업 가이드</a>` | JavaScript 클릭 이벤트만 있고 href 없음 |
| 설명형 앵커 | `Product schema와 merchant feed 점검법` | `여기`, `자세히`, `클릭` 반복 |
| 허브 연결 | 개념 → 실습 → 사례 → 제품 문서가 이어짐 | 좋은 글이 고립 페이지로 남음 |
| 공개 접근 | 로그인 없이 핵심 요약 접근 가능 | 권한/지역/세션 조건 뒤에 본문 숨김 |

## 핵심 URL 발견 경로를 기록하는 법

![AI 크롤러 접근 경로 지도](../assets/images/body-figures/halox-geo-06-04-ai-crawler-access-route-map-codex-only.png)

_robots 허용 여부만 보지 말고 sitemap, 내부 링크, 핵심 URL 접근 경로를 함께 기록해야 합니다._


AI 답변 근거가 되어야 할 URL은 robots 허용만으로 충분하지 않습니다. sitemap, 내부 링크, 외부 source, 뉴스룸/허브 페이지에서 반복적으로 발견될 수 있어야 합니다.

| 발견 경로 | 확인 질문 | 좋은 상태 |
|---|---|---|
| sitemap | 핵심 URL과 lastmod가 들어 있는가? | 제품/가이드/뉴스룸/리포트 URL 포함 |
| 내부 링크 | 허브와 관련 글에서 href로 연결되는가? | 본문 문맥 링크와 breadcrumb 존재 |
| 외부 source | 파트너/언론/디렉터리에서 대표 URL을 가리키는가? | 오래된 캠페인 URL이 아님 |
| robots | 경로가 차단되지 않는가? | 공개 콘텐츠 경로 허용 |
| HTTP status | 200/301이 안정적인가? | 404/302/체인 리다이렉트 없음 |
