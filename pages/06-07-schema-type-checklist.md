...
| Person | 대표/전문가/저자 소개, 인터뷰, 칼럼 저자 페이지 | 이름, 직함, 전문 분야, 소속, 프로필 URL, 공개 가능한 이력 | 누가 말하는지, 어떤 전문성을 가진 사람인지 설명하기 위해 |
...
| contactPoint | 공개 가능한 연락처/문의 채널 | 내부 연락처나 더 이상 쓰지 않는 번호 |
...
인물/전문가 페이지에서는 E-E-A-T를 흉내 내는 문구보다 공개 가능한 이력, 저자 페이지, 실제 작성 글, 인터뷰, 발표 자료 같은 연결 근거가 중요합니다. 의료/금융/법률처럼 규제가 있는 업종은 자격, 경력, 후기 표현이 법적 기준과 충돌하지 않는지도 함께 봐야 합니다.
...
Product schema는 커머스 GEO와 AI 구매 에이전트 대응에서 특히 중요합니다. Google은 상품 구조화 데이터가 검색 중인 잠재 구매자에게 상품 정보를 더 잘 보여 주는 데 도움이 될 수 있다고 설명합니다. 다만 실제 구매 판단에서는 Product schema, 상세페이지, merchant feed, 배송/반품 정책이 함께 맞아야 합니다.
...
| 대표/전문가 프로필 | Person, ProfilePage | 이름/직함/전문 분야/소속 | 프로필 URL과 sameAs 확인 | 공개 가능한 이력만 구조화 |
...
공개 가능한 정보만 넣습니다. 이름, 직함, 소속, 전문 분야, 공식 프로필처럼 본문에 보이는 정보가 기준입니다. 민감한 개인정보나 검증되지 않은 경력은 넣지 않습니다.
