...
메타 정보 점검까지 끝나면 6장 기술 점검은 `발견 가능성 → 렌더링 → URL 안정성 → 크롤러 접근성 → schema/내부 링크 → 공식 도구 검증 → 메타/canonical → 사이트링크/리치 리절트 구분` 흐름으로 닫힙니다. 검색결과 아래 작은 링크와 구조화 데이터 리치 리절트를 구분해야 한다면 [06-09. 사이트링크와 리치 리절트 차이](pages/06-09-sitelinks-rich-results-section-links.md)를 이어서 봅니다. 이후에는 [07. 산업별 GEO 전략](https://wikidocs.net/346335)으로 넘어가 업종별로 어떤 기술 요소를 우선해야 하는지 봅니다.
