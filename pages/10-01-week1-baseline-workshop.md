...
1주차는 좋은 점수를 만드는 시간이 아니라 현재 상태를 같은 기준으로 기록하는 시간입니다. 질문셋, 경쟁사, AI 답변, 답변 근거(source)와 화면 인용(citation)을 처음부터 같은 형식으로 남겨야 4주 뒤 변화가 보입니다.

따라서 1주차에는 도구 사용법보다 “무엇을 같은 방식으로 반복 측정할 것인가”를 먼저 정해야 합니다. 이 기준이 없으면 2~4주차 실행 결과도 개선인지 단순 변동인지 판단하기 어렵습니다.
...
| 결과 | mention, 답변 근거(source), 화면 인용(citation), 경쟁사 |
| 해석 | 2주차에 확장할 대표 질문 |

## 기준선 기록표

| 질문 | 모델 | 우리 브랜드 mention | 답변 근거 | 화면 인용 URL | 경쟁사 | 메모 |
|---|---|---|---|---|---|---|
|  |  | 있음/없음 | 있음/없음 |  |  |  |
|  |  | 있음/없음 | 있음/없음 |  |  |  |
|  |  | 있음/없음 | 있음/없음 |  |  |  |
...
측정 모델:
우리 브랜드가 언급된 질문 수:
답변 근거(source)와 화면 인용(citation)이 확인된 질문 수:
자주 등장한 경쟁사:
2주차에 확장할 대표 질문 3개:
1주차 인사이트 3개:
```
