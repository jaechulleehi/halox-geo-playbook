...
[Schema와 내부 링크](https://wikidocs.net/346394)에서 본 것처럼 schema는 본문을 대체하지 않습니다. 커머스에서는 이 원칙을 더 먼저 봅니다. 가격, 재고, 옵션, 배송, 반품처럼 실제 거래에 영향을 주는 정보가 서로 다르면 AI는 무엇을 믿어야 할지 판단하기 어렵습니다.

[TOC]

## Product schema가 보완하는 것

Product schema는 상품 페이지의 핵심 정보를 구조화된 형식으로 전달합니다. 검색 엔진과 AI 시스템이 상품명, 브랜드, 이미지, SKU, 가격, 재고, 리뷰, 평점 등을 더 안정적으로 이해하는 데 판단 기준이 됩니다.
...
## feed/schema/본문 충돌을 우선순위화하기

모든 불일치가 같은 위험은 아닙니다. 구매 피해와 AI 오해 가능성이 큰 항목부터 고쳐야 합니다.

| 충돌 항목 | 위험도 | 우선 조치 |
|---|---|---|
| 가격 | 매우 높음 | feed/schema/본문/광고 소재 즉시 동기화 |
| 재고 | 매우 높음 | availability와 품절 표시 점검 |
| 배송/반품 | 높음 | 정책 페이지와 Product schema 연결 |
| 상품명/SKU | 높음 | 옵션별 식별자 정리 |
| 리뷰/평점 | 중간~높음 | 실제 리뷰 수와 텍스트 근거 확인 |
| 이미지/설명 | 중간 | alt와 주요 스펙 보강 |

![schema와 feed 충돌 우선순위 맵](../assets/images/body-figures/halox-geo-11-03-schema-feed-conflict-priority-map-codex-only.png)

<small>가격, 재고, 배송, 반품처럼 고객 피해로 이어지는 충돌은 본문, schema, feed, 정책 문서를 함께 맞춰야 한다.</small>
