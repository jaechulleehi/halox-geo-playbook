...
먼저 상품 상세페이지에는 이 상품이 무엇이고 누구에게 맞는지 보여야 합니다. Product/Offer/Review 같은 구조화 데이터는 본문과 같은 값을 말해야 합니다. merchant feed는 가격, 재고, 배송, 반품 정보를 최신으로 전달해야 합니다. 정책 문서는 배송/반품/환불/보증 조건을 명확히 설명해야 하고, 외부 리뷰와 비교 글은 상품을 믿고 비교할 근거가 됩니다.
...
가상 커머스 브랜드 AcmeStore라면 `우리 상품이 추천됐는가`보다 `조건형 구매 질문에서 정확한 상품/가격/정책/source로 설명됐는가`를 봐야 합니다. 상품명은 맞지만 가격이 오래됐거나, 재고가 틀리거나, 반품 정책이 잘못 설명되면 좋은 노출이 아니라 위험한 노출입니다.

## 우선순위는 상품군부터 좁힌다

모든 상품을 동시에 고치려고 하면 범위가 너무 커집니다. 먼저 AI 답변에 들어갈 가능성이 높거나 오류가 났을 때 비용이 큰 상품군부터 봅니다.

- 매출 상위 상품: 가격/재고/배송/schema/feed를 먼저 맞춥니다.
- 광고 집행 상품: 랜딩 URL, canonical, 가격 충돌을 확인합니다.
- 비교가 잦은 상품: 비교표, 리뷰, 경쟁 상품 기준을 정리합니다.
- 반품/문의 많은 상품: 정책, FAQ, CS 문서를 보강합니다.
- 신제품/캠페인 상품: 출시일, 가격, 옵션, 종료 URL을 관리합니다.

## 이 장을 읽는 순서

[11-01. AI 구매 에이전트와 커머스 GEO 검색 변화](https://wikidocs.net/346597)에서는 SEO/SERP, AIO, GPT/Claude/Perplexity 답변 시장이 커머스에서 어떻게 갈라지는지 봅니다. [11-02. 커머스 GEO 상품 정보 구조화](https://wikidocs.net/346598)에서는 AI 고객이 상품 정보를 어떤 순서로 확인하는지 정리합니다.

[11-03. Product schema와 merchant feed 점검](https://wikidocs.net/346599)에서는 Product schema와 merchant feed가 무엇을 보완하고 어디서 충돌하는지 봅니다. [11-04. 커머스 GEO 리스크 2주 점검](https://wikidocs.net/346600)에서는 리스크와 2주 점검 체크리스트로 실제 실행 순서를 만듭니다.
...
구현 기준은 Google의 [Product structured data](https://developers.google.com/search/docs/appearance/structured-data/product), [Merchant Center 제품 데이터 사양](https://support.google.com/merchants/answer/7052112), schema.org의 [Product](https://schema.org/Product), [Offer](https://schema.org/Offer), [MerchantReturnPolicy](https://schema.org/MerchantReturnPolicy)를 함께 확인합니다. 이 장은 공식 문서를 그대로 옮기기보다 실무자가 어떤 값을 서로 맞춰야 하는지에 집중합니다.
